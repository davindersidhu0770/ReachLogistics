...
class PickingBatchScannerScreen extends StatefulWidget {
  ...
  const PickingBatchScannerScreen({super.key, required this.items});

  @override
  State<PickingBatchScannerScreen> createState() =>
      _PickingBatchScannerScreenState();
}
...
class _PickingBatchScannerScreenState extends State<PickingBatchScannerScreen> {
  ...
  Future<void> _handleScan(String code) async {
    ...
    // ── Step 2: bulk second scan ──────────────────────────────────────────
    if (_waitingForBulkScan && _activeItem != null) {
      setState(() {
        _isProcessing = true;
        _statusMsg = null;
      });
      final result = await _service.scanSecond(_activeItem!.orderItemId, code);
      if (!mounted) return;
      setState(() => _isProcessing = false);

      if (result.success && result.isCompleted) {
        _markDone(_activeItem!);
    ...
    // ── Step 1: match item by orderItemId ─────────────────────────────────
    ...
    if (_doneIds.contains(scannedId)) {
      _setStatus("Already completed", error: true);
      return;
    }
    ...
    final result = await _service.scanFirst(item.orderItemId);
    if (!mounted) return;
    setState(() => _isProcessing = false);
    ...
    if (result.isBulk) {
      ...
      _setStatus("Bulk item — scan second barcode");
    }
  }
  ...
  void _openManualEntry() {
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        title:
            Text(_waitingForBulkScan ? "Enter Second Item UID" : "Enter Item Barcode"),
        content: TextField(
          controller: _manualController,
          autofocus: true,
          keyboardType:
              _waitingForBulkScan ? TextInputType.text : TextInputType.number,
  ...
  Widget build(BuildContext context) {
    ...
    final accent =
        _waitingForBulkScan ? const Color(0xFF6A1B9A) : const Color(0xFF1565C0);
                          ...
                          AnimatedSwitcher(
                            ...
                            child: _isProcessing
                                ...
                                : Text(
                                    _buildStatusText(),
                                    key: ValueKey(
                                        '$_statusMsg$_waitingForBulkScan${_activeItem?.orderItemId}'),
                    ...
                    SizedBox(
                      ...
                      child: ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          final isDone =
                              _doneIds.contains(item.orderItemId);
                          final isActive =
  ...
  String _buildStatusText() {
    ...
    if (_waitingForBulkScan) return "Scan second barcode";
